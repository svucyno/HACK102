...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If the user's income is already greater than 0, they likely completed onboarding previously
      if (data.user && data.user.income > 0) {
    ...
    <div className="min-h-screen flex items-center justify-center p-6 relative">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          <Logo size="medium" />
          <h2 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary mt-6">Welcome Back</h2>
          <p className="text-muted-foreground text-sm mt-2 focus:text-white">Enter your details to access your dashboard</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          >
            {loading ? <span className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></span> : <>Login <LogIn className="w-4 h-4" /></>}
          </button>
        </form>

        <div className="mt-6 text-center text-sm border-t border-white/10 pt-6">
          <span className="text-muted-foreground cursor-default">New user? </span>
          <Link to="/register" className="text-secondary hover:text-secondary-foreground hover:underline inline-flex items-center gap-1 font-medium">
            Register for an account <ArrowRight className="w-3 h-3" />
          </Link>
        </div>
      </motion.div>
    </div>
